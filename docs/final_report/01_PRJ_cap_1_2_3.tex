...
\begin{itemize}
    \item \textbf{\gls{git} e Github}: Para assegurar o controlo de versões de todo o código produzido, foi utilizado o sistema \gls{git}, em conjunto com a plataforma Github. Esta combinação permitiu não só manter um histórico rigoroso das alterações realizadas, mas também garantir a colaboração, e a integridade do desenvolvimento ao longo do tempo \cite{git,github}.

    \item \textbf{Notion}: A gestão de tarefas e a organização dos fluxos de trabalho foram centralizadas no Notion. Esta ferramenta revelou-se extremamente versátil para planear atividades, definir prioridades, acompanhar o progresso das diferentes fases do projeto e garantir que todas as entregas fossem realizadas de acordo com os prazos estabelecidos \cite{notion}.

    \item \textbf{\gls{vscode}}: Como editor de código, foi utilizado o \gls{vscode}, um ambiente de desenvolvimento integrado leve, mas altamente extensível. Através da instalação de extensões específicas, foi possível adaptar o \gls{vscode} às necessidades concretas do projeto, potenciando a produtividade e melhorando a experiência de programação \cite{vscode}.
\end{itemize}

Estas ferramentas, utilizadas de forma integrada e consistente, desempenharam um papel decisivo para a boa execução do projeto, não só do ponto de vista técnico, mas também no que respeita à organização, planeamento e comunicação entre todas as partes envolvidas.
