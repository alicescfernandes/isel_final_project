...
class PlotlyChart extends HTMLElement {
    ...
    renderQuarterNavigation() {
        ...
        select.innerHTML = `
        <button ${!quarter.prev && 'disabled'} aria-label="Previous Quarter" class="prev-quarter chart-quarter-text">←</button>
        <button ${!quarter.next && 'disabled'} aria-label="Next Quarter" class="next-quarter chart-quarter-text">→</button>`
    }

    renderSpinner() {
        this.innerHTML = `<div role="status" class="spinner-container">
    ...
    setupLazyLoad() {
        const observer = new IntersectionObserver((entries, obs) => {
            ...
            root: null,         // viewport
            rootMargin: "100px",  // margem extra
            threshold: 0.1      // % visível para ativar (10%)
        });
